...
/**
 * Provides a singleton instance of [HomeClient]. This class is responsible for creating and
 * managing the [HomeClient] instance, including handling account switching.
 *
    ...
    val client =
      measureTimedValue {
          if (userId.isEmpty()) {
            ...
            Home.getClient(
              applicationContext,
              account = lazy { UserAccount.GoogleAccount(account = Account(userId, "com.google")) },
              homeConfig = config,
            )
          }
        }
        .also { Log.i(TAG, "HomeSDK construction took ${it.duration.inWholeMilliseconds} ms.") }
