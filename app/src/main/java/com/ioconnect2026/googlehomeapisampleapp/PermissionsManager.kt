...
class PermissionsManager(
  ...
  private fun checkPermissions() {
    scope.launch {
      ...
      client.hasPermissions().collectLatest { state ->
        ...
        if (!isInitialized.value) isInitialized.emit(true)
      }
    }
  }
   ...
   *
   * This function should be used by external components (e.g., UI layers) to refresh the permission
   * state. Directly calling checkPermissions() is discouraged to maintain encapsulation and allow
   * future flexibility.
   */
  ...
  fun requestPermissions(isForceRefresh: Boolean = false) {
    scope.launch {
      try {
        ...
        val result: PermissionsResult = client.requestPermissions(ForcePermissionFlow.FORCE_LAUNCH)
        ...
        MainActivity.showError(this, e, "Permissions request failed")
      }
    }
  }
  ...
  private fun reportPermissionState(permissionState: PermissionsState) {
    ...
    when (permissionState) {
      PermissionsState.GRANTED -> MainActivity.showDebug(this, message)

      PermissionsState.NOT_GRANTED -> MainActivity.showWarning(this, message)

      PermissionsState.PERMISSIONS_STATE_UNAVAILABLE -> MainActivity.showWarning(this, message)

      PermissionsState.PERMISSIONS_STATE_UNINITIALIZED -> MainActivity.showError(this, message)
    }
  }
  ...
  private fun reportPermissionResult(permissionResult: PermissionsResult) {
    ...
    if (permissionResult.errorMessage != null) message += " | " + permissionResult.errorMessage
    // Report the permission result:
    when (permissionResult.status) {
      PermissionsResultStatus.SUCCESS -> MainActivity.showDebug(this, message)

      PermissionsResultStatus.CANCELLED -> MainActivity.showWarning(this, message)

      PermissionsResultStatus.ERROR -> MainActivity.showError(this, message)
    }
  }
}
